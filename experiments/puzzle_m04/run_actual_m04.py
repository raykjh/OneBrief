"""Run PUZZLE M04 continuously from the verified M01-M03 chain."""
...
from onebrief.godot_product_execution import execute_godot_m04
from onebrief.godot_product_finish import GodotProductFinishPlan
from onebrief.godot_successor_execution import GodotSuccessorAuthority, _sha
...
def main() -> int:
    ...
    parser.add_argument("--plan", type=Path, default=Path(__file__).with_name("product_plan.json"))
    ...
    if generated.qualification is None or generated.qualification.status != "passed":
        raise RuntimeError("PUZZLE M04 ToolPack qualification did not pass")
    ...
    if not approved.execution_ready or approved.generated is None:
        raise RuntimeError("PUZZLE M04 ToolPack is not execution-ready")
    ...
    milestone_plan_sha256 = hashlib.sha256(outcome + b"\0M04:player-facing-product-finish").hexdigest()
    ...
    authority_digest = _sha({
        ...
        "trusted_compiler_sha256": approved.generated.trusted_component_digests["godot_product_finish_compiler"],
        "budget_usd": 0, "preserve_receipt_chain": True,
    })
    result = execute_godot_m04(
        GodotSuccessorAuthority(
            project_id=imported.record.manifest.project_id,
            source_revision=source_revision,
            toolpack_sha256=approved.generated.sha256,
            milestone_plan_sha256=milestone_plan_sha256,
            authority_envelope_sha256=authority_digest,
            predecessor_result_dirs=[str(args.m01_result.resolve()), str(args.m02_result.resolve()), str(args.m03_result.resolve())],
        ),
        GodotProductFinishPlan.model_validate_json(args.plan.read_text(encoding="utf-8")),
