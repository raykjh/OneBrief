...
def request_fingerprint(intake: IntakeRequest) -> str:
    ...
    for source in intake.internal_sources:
        # Project continuation is generated by OneBrief and necessarily changes
        # after every attempt. Treating that audit snapshot as user intent makes
        # the same goal look new forever. Project ID and the separately verified
        # repository HEAD still gate all compatible code reuse; user-supplied
        # sources remain digest-bound below.
        if (
            source.name.casefold().startswith("onebrief-project-continuation-")
            and "existing_project_continuation" in source.requirement_keys
        ):
            continue
