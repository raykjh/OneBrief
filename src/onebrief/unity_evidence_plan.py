...
def render_unity_evidence_journey(
    ...
    *,
    ...
    proxy_authority_contract: tuple[str, str] | None = None,
    ...
    proxy_authority = (
        [
            "        private const string KhalinosProxyAuthenticationAuthority =",
            f"            {_cs(proxy_authority_contract[0] + ' -> ' + proxy_authority_contract[1])};",
            "",
        ]
        if proxy_authority_contract is not None
        else []
    )
    source = "\n".join([
    ...
    assembly = json.dumps({
        ...
        # TestAssemblies supplies both TestRunner references. Listing them
        # again is a hard duplicate-reference error in Unity 6000.3.
