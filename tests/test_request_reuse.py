...
def test_budget_gated_continuation_reuses_verified_snapshot_provenance(
    tmp_path: Path,
) -> None:
    head = "8" * 40
    job = tmp_path / "budget-gated"
    (job / "inputs").mkdir(parents=True)
    snapshot = job / "work" / "project_snapshot"
    snapshot.mkdir(parents=True)
    (job / "inputs" / "intake.json").write_text(
        _intake().model_dump_json(), encoding="utf-8"
    )
    (job / "job.json").write_text(
        json.dumps({"job_id": "budget-gated", "status": "needs_budget"}),
        encoding="utf-8",
    )
    (snapshot / "restore_evidence.json").write_text(
        json.dumps({
    ...
    (job / "work" / "code_change_set.json").write_text(
        '{"summary":"preserved candidate"}', encoding="utf-8"
    )
    (job / "work" / "development_verification_failure.txt").write_text(
    ...
    assert candidate is not None
    assert candidate.job_id == "budget-gated"
    assert "code_change_set.json" in candidate.reusable_artifacts
