...
- Writer creates the artifact from the contract and findings but cannot approve it.
- Verifier is independent, checks every acceptance criterion, and returns `PASS`,
  `REVISE`, or `NEEDS_INFORMATION`.
- Revision Agent applies only the verifier's blocking instructions. Its result always
  returns to the Verifier before completion.
